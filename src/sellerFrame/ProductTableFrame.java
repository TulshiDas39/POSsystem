...
public class ProductTableFrame extends JFrame {
	...
	public ProductTableFrame() throws SQLException {
		...
		JLabel lblCategorisOfProducts = new JLabel("Product Table");
		lblCategorisOfProducts.setFont(new Font("Californian FB", Font.BOLD, 21));
		lblCategorisOfProducts.setBounds(637, 52, 135, 26);
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			//	dispose();
				...
				int id =Integer.valueOf(JOptionPane.showInputDialog("Enter Product id to delete")).intValue();
				if(a.SearchProductId(id)){
				
				new ProductTable().delete(id);
				}
				else{
		...
		btnSearch.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				boolean t = new ProductTable().SearchProductId(id);
				if(t){
					new ProductTable().searchProductInformation(id);
				}
				else{
					JOptionPane.showMessageDialog(null, "Id is not in the table","error",JOptionPane.ERROR_MESSAGE);
				}
	...
	private void createTableOfProducts() throws SQLException {
		...
		table.setModel(new DefaultTableModel(getObjectOfData()
			...
			new String[] {
				"Product Id", "Product Name", "Category", "Price","Unit", "Quantity"
			}
		) {
			Class[] columnTypes = new Class[] {
				Object.class, Object.class, Object.class, Double.class, Object.class,Double.class
			};
	...
	public Object [][] getObjectOfData() throws SQLException{
		ResultSet rs = null;
		rs=new ProductTable().getAllData();
		//rs.last();
		...
		while(rs.next()){
			...
			obj[k][3]=rs.getDouble(4);
			obj[k][4]=rs.getString(5);
			obj[k][5]=rs.getDouble(6);
			k++;
		}
		return obj;
	}
